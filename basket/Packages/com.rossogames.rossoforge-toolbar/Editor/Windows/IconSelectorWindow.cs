...
namespace Rossoforge.Toolbar.Editor.Windows
{
    public class IconSelectorWindows : EditorWindow
    {
        ...
        private const string _title = "Unity Icons";
        ...
        public static void ShowPopup(Action<string> onIconSelectedCallback)
        {
            IconSelectorWindows window = CreateInstance<IconSelectorWindows>();
            InitializeWindow(window);
            window.OnIconSelected = onIconSelectedCallback;
            window.ShowUtility();
        }

        private static void InitializeWindow(IconSelectorWindows window)
        {
            window.titleContent = new GUIContent(_title);
            window.position = new Rect(Screen.width / 2, Screen.height / 2, 700, 400);
        }
        ...
        private void DrawIcons()
        {
            ...
            iconsPerRow = Mathf.FloorToInt(windowWidth / (iconSize + padding));
            ...
            {
                string iconName = _iconNames[i];

                if (!string.IsNullOrWhiteSpace(_searchValue) && !iconName.ContainsInvarianCulture(_searchValue))
                    continue;
